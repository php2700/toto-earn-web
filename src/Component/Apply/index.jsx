...
export default function Apply() {
  ...
  // --- New State for Scratch Card Logic ---
  ...
  // Points claim karne ki API function
  const handleScratchComplete = async () => {
    try {
      // Yahan hum backend API call karenge points ko wallet me add karne ke liye
      // Maan lete hain ki backend se hume 'pendingPoints' mil rahe hain
      if (!userData?.pendingPoints || userData?.pendingPoints <= 0) return;

      const res = await axios.post(
        `${import.meta.env.VITE_APP_API_BASE_URL}api/user/claim-scratch-points`,
        { userId },
        { headers: { Authorization: `Bearer ${token}` } }
      );

      if (res.status === 200) {
        toast.success(`${userData?.pendingPoints} Points added to wallet!`);
        setHasScratched(true); // Scratch card ko khali karne ke liye
        dispatch(fetchUserData()); // Wallet balance refresh karne ke liye
      }
    } catch (error) {
      console.error("Scratch Error:", error);
      toast.error("Failed to add points to wallet");
    }
  };
  ...
  return (
    <section className="w-full pt-16">
      <div className="h-[50vh] flex items-center justify-center bg-grid" style={{ backgroundImage: `url(${bgImg})` }}>
        <h1 className="text-6xl font-bold text-gray-800 uppercase tracking-widest">REFER & EARN</h1>
      </div>

      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4">
        <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-3xl">
          <h1 className="text-2xl font-bold text-center mb-4">
            Refer & Earn – Get Reward Points! 💰
          </h1>
          <p className="text-center text-gray-600 mb-4 text-sm">
            Refer your friends and earn points! 💸 One-time activation fee of ₹100. 
            Once your friend joins, points will be generated for up to 30 days. 
            <strong> Note:</strong> A daily 24-hour limit applies for earning generation. 
            Scratch your reward card and add points to your wallet instantly!
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6 text-center">
            <div>
              <div className="rounded-full inline-flex items-center justify-center mb-3 mx-auto" style={{ width: 64, height: 64 }}><img src={googleImg} alt="Google" /></div>
              <p className="text-gray-800 text-sm"><strong>Login / Sign Up</strong><br />1-click Google Login.</p>
            </div>
            <div>
              <div className="inline-flex items-center justify-center mb-3 mx-auto" style={{ width: 64, height: 64 }}><img src={account} alt="Payment" /></div>
              <p className="text-gray-800 text-sm"><strong>Activate Account ₹100</strong><br />Unlock your referral link instantly.</p>
            </div>
            <div>
              <div className="rounded-full inline-flex items-center justify-center mb-3 mx-auto " style={{ width: 64, height: 64 }}><img src={share} alt="Share" /></div>
              <p className="text-gray-800 text-sm"><strong>Share & Earn Points</strong><br />Share link and get points for 30 days for every successful referral.</p>
            </div>
          </div>

          <h2 className="w-full bg-blue-500 text-center text-semibold text-white py-2 rounded-t-lg">
            Wallet & Referral Overview
          </h2>

          {/* --- Golden Box Scratch Card Container --- */}
          <div className="w-full bg-gradient-to-r from-yellow-400 via-yellow-500 to-yellow-600 p-8 mb-6 flex flex-col items-center justify-center shadow-inner">
            <h3 className="text-white font-bold mb-4 uppercase tracking-wider drop-shadow-md text-lg">Your Reward Scratch Card</h3>
            
            {/* Jab tak scratch nahi hua aur points hain tabhi card dikhega */}
            {!hasScratched && userData?.pendingPoints > 0 ? (
              ...
              <div className="h-[150px] w-[280px] bg-white/20 border-2 border-dashed border-white flex items-center justify-center rounded-lg">
                <p className="text-white font-medium text-center px-4 italic">
                  {hasScratched ? "Points added to wallet successfully!" : "No new rewards available right now."}
                </p>
              </div>
            )}
            <p className="text-white text-xs mt-3 opacity-80 font-semibold">Scratch to reveal your daily points</p>
          </div>

          <div className="grid grid-cols-2 gap-4 text-center mb-4">
            <div><p>Wallet Balance <br /><span className="font-bold">{userData?.walletAmount}</span></p></div>
            <div><p>Active Referrals <br /><span className="font-bold">{userData?.activeReferralsCount}</span></p></div>
            <div><p>Pending Referrals <br /><span className="font-bold">{userData?.inactiveReferralsCount}</span></p></div>
            <div><p>Min. Withdrawal <br /><span className="font-bold">₹200</span></p></div>
          </div>

          <div className="text-center mb-4">
            {userData?.isActivate && <p className="font-medium">REFERRAL CODE : {userData?.referralCode}</p>}
            <div className="flex flex-col md:flex-row justify-center space-x-4 m-2 gap-4 items-center">
              <button
                disabled={!userData?.isActivate}
                className={`flex items-center gap-2 py-1 px-3 rounded ${userData?.isActivate ? "bg-green-500 hover:bg-green-600 text-white cursor-pointer" : "bg-gray-300 text-gray-500 cursor-not-allowed"}`}
                onClick={() => {
                  const refLink = `${import.meta.env.VITE_WEBSITE_URL}/signup?ref=${userData.referralCode}`;
                  window.open(`https://wa.me/?text=${encodeURIComponent("Join using my link: " + refLink)}`, "_blank");
                }}
              >
                <img src="https://cdn-icons-png.flaticon.com/512/733/733585.png" alt="WhatsApp" className="w-4 h-4" /> WhatsApp
              </button>
              <button disabled={!userData?.isActivate} onClick={copyReferral} className={`flex items-center gap-2 py-1 px-3 rounded transition ${userData?.isActivate ? "bg-gray-500 hover:bg-gray-600 text-white cursor-pointer" : "bg-gray-300 text-gray-500 cursor-not-allowed"}`}>
                <img src="https://cdn-icons-png.flaticon.com/512/60/60990.png" alt="Copy" className="w-4 h-4" /> {copied ? "Copied" : "Copy"}
              </button>
              <button onClick={handleInst} disabled={!userData?.isActivate} className={`flex items-center gap-2 py-1 px-3 rounded transition ${userData?.isActivate ? "bg-pink-500 hover:bg-pink-600 text-white cursor-pointer" : "bg-gray-300 text-gray-500 cursor-not-allowed"}`}>
                <img src="https://cdn-icons-png.flaticon.com/512/1384/1384063.png" alt="Instagram" className="w-4 h-4" /> Instagram
              </button>
            </div>
          </div>
          ...
          <button
            onClick={() => setShowModal(true)}
            disabled={userData?.walletAmount < 200}
            className={`w-full py-2 rounded-lg mb-4 text-white ${userData?.walletAmount >= 200 ? "bg-blue-500 hover:bg-blue-600 cursor-pointer" : "bg-gray-400 cursor-not-allowed"}`}
          >
            Withdraw
          </button>
          <p className="text-center text-gray-600 text-sm">Total Withdraw : {userData?.totalAmount}</p>
        </div>
      </div>

      {/* Baaki ke Modals (showModal, showQR, paymentModel) ka code same rahega isliye maine use choda hai... */}
      {/* ... code continue from your original modals ... */}
    </section>
  );
}